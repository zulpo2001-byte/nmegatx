...
/**
 * Global_Card_Adapter_Gateway v7.1
 *
 * 认证变更（v7.1）：
 *   旧：请求头 X-API-Key（明文）+ X-API-Secret（明文 secret）
 *   新：X-Api-Key + X-Timestamp + X-Signature（HMAC，secret 不出网）
 *       签名公式：HMAC-SHA256(api_key + "\n" + ts + "\n" + sha256(body), api_secret)
 *
 * 配置字段变更（v7.1）：
 *   旧：nme_url / api_key / api_secret（4个，独立 whsec 字段）
 *   新：nme_url / api_key / api_secret（3个，api_secret 同时担任 whsec 回调验签密钥）
 *   一码配置串自动填充，无需手动复制密钥。
 *
 * 一码配置串格式（从 NME 后台复制）：
 *   Base64({ "v":2, "type":"a", "nme":"https://...", "ak":"ak_cb_xxx", "whsec":"whsec_xxx" })
 */
class Global_Card_Adapter_Gateway extends WC_Payment_Gateway
{
    ...
    {
        $this->id                 = 'global_card_adapter';
        $this->method_title       = 'Global Card Adapter (Stripe via NME)';
        $this->method_description = 'A站Stripe支付网关 v7.1。统一HMAC签名，Secret不出网，支持一码配置串。';
        ...
        $this->form_fields = [
            ...
            'nme_info' => [
                ...
                'description' =>
                  ...
                  . '<code id="gca_cb_url" style="flex:1;padding:6px 8px;background:#fff;border:1px solid #8c8f94;border-radius:3px;font-size:13px;">'
                  . esc_html($callback_url) . '</code>'
                  . '<button type="button" id="gca_copy_cb_btn" class="button button-secondary">复制</button>'
                  . '</div></div>',
            ],

            // ── 手动填写（配置串失效时备用）──────────────
            'manual_info' => [
                'title'       => '🔑 手动填写（配置串自动填充后无需修改）',
                'type'        => 'title',
    ...
    private function render_config_string_input(): string
    {
        return '<div style="background:#f0f7f0;border:1px solid #4CAF50;border-radius:4px;padding:14px;">'
             ...
             . '<textarea id="gca_config_string" rows="3" style="flex:1;font-family:monospace;font-size:12px;padding:6px;border:1px solid #8c8f94;border-radius:3px;resize:vertical;" placeholder="粘贴配置串（Base64字符串）..."></textarea>'
             . '<button type="button" id="gca_parse_config_btn" class="button button-primary" style="white-space:nowrap;">解析填充 →</button>'
             . '</div>'
             . '<p id="gca_config_status" style="margin:6px 0 0;font-size:12px;color:#666;"></p>'
             . '</div>';
    }
    ...
    public function process_payment($order_id): array
    {
        ...
        $order->update_meta_data('_gca_nme_order_id', $body['order_id'] ?? '');
        $order->update_status('pending', '已提交NME，等待支付完成。');
        $order->save();
        WC()->cart->empty_cart();
    ...
    public function inject_admin_scripts(): void
    {
        ...
        jQuery(document).ready(function($) {
            ...
            // ── 复制回调地址 ──
            $('#gca_copy_cb_btn').on('click', function() {
                var text = $('#gca_cb_url').text().trim();
            ...
            // ── 配置串解析自动填充 ──
            $('#gca_parse_config_btn').on('click', function() {
                var raw = $('#gca_config_string').val().trim();
                if (!raw) {
                    $('#gca_config_status').css('color','#c00').text('请先粘贴配置串。');
                    return;
                }
                try {
                    // Base64 解码
                    ...
                    if (cfg.type !== 'a') {
                        $('#gca_config_status').css('color','#c00').text('错误：这是 B站配置串，请使用 A站配置串。');
                        return;
                    }
                    if (cfg.v !== 2) {
                        $('#gca_config_status').css('color','#f90').text('警告：配置串版本不匹配（v' + cfg.v + '），尝试继续填充…');
                    }

                    if (cfg.nme)   $('#woocommerce_global_card_adapter_nme_url').val(cfg.nme);
                    if (cfg.ak)    $('#woocommerce_global_card_adapter_api_key').val(cfg.ak);
                    if (cfg.whsec) $('#woocommerce_global_card_adapter_api_secret').val(cfg.whsec);

                    $('#gca_config_status').css('color','#4CAF50').text('✓ 配置串解析成功，字段已自动填充，请点击「保存更改」。');
                } catch(e) {
                    $('#gca_config_status').css('color','#c00').text('解析失败：配置串格式错误，请重新从NME后台复制。');
                }
            });

            // 支持直接粘贴时自动触发解析
            $('#gca_config_string').on('paste', function() {
                setTimeout(function() { $('#gca_parse_config_btn').trigger('click'); }, 100);
            });
        });
        </script>
        <?php
